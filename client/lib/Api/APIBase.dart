...
class APIBase {
  ...
  Future<dynamic> postRequest(String url, dynamic bodyParam, 
    ...
    try{
      if(headers == null){
        response = await http.post(uri,body: bodyParam).timeout(Duration(seconds: 120));
      } else {
        response = await http.post(uri,body: bodyParam, headers: headers).timeout(Duration(seconds: 120));
      }
